...
@php
    // No `capitalize` here: these badges carry real sentences -- a
    // deactivation reason, a consent state -- and title-casing them reads
    // as a bug. Callers that want it can pass the class.
...
<span {{ $attributes->merge(['class' => "$baseStyles $sizeClass $colorStyles"]) }}>
    @if($startIcon)
        {!! $startIcon !!}
    @endif
